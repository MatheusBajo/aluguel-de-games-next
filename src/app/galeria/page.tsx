...
export default async function GaleriaPage() {
    ...
    const totalCategorias = new Set(catalogo.map((i) => i.key.split('/')[0])).size;
        ...
        <main className="relative overflow-hidden">
            ...
            <section className="relative mx-auto max-w-7xl px-4 py-16">
                <div className="grid gap-px bg-border/40 grid-cols-2 md:grid-cols-4 rounded-2xl overflow-hidden border border-border/40">
                    {/* Contagem real do catálogo (build time). Saíram "500+ Eventos"
                        e "98% Satisfação", que ninguém mediu. Ver src/lib/sales-utils.ts */}
                    {[
                        { to: anos, suffix: "+", label: "Anos de estrada" },
                        { to: totalEquipamentos, suffix: "", label: "Equipamentos" },
                        { to: totalCategorias, suffix: "", label: "Categorias" },
                        { to: 1993, suffix: "", label: "Desde" },
                    ].map((s, i) => (
